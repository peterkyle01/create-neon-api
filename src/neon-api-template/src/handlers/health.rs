...
pub async fn health_check(State(config): State<Arc<Config>>) -> Result<impl axum::response::IntoResponse, AppError> {
    ...
    match reqwest::Client::new().get(&config.data_api_url).send().await {
