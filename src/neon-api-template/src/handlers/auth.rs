...
pub async fn sign_out(State(config): State<Arc<Config>>) -> Result<impl axum::response::IntoResponse, AppError> {
    let mut client = NeonClient::new(&config);
...
fn map_auth_error(err: anyhow::Error) -> AppError {
    ...
    let status_code = msg.split(": ").nth(1).and_then(|s| s.split(' ').next()).and_then(|s| s.parse::<u16>().ok());
